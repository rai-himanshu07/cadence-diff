...
def test_focus_document_uses_native_object_handle_and_visible_foreground_handle(
    ...
    from qc_tool.focus import excel_focus, win32_office

    window, _application, _workbook = _window()
    acquired: list[int] = []
    foreground: list[int] = []
    monkeypatch.setattr(excel_focus.sys, "platform", "win32")
    monkeypatch.setattr(
        win32_office,
        "object_from_window",
    ...
    document = OpenDocument(
        ...
        saved=True,
        autosave=False,
        object_model_window_handle=707,
    )
